#line 1 "./table/table2D/cfunc.mod"
...
static void
cm_table2D_callback(Mif_Private_t *mif_private, Mif_Callback_Reason_t reason)
{
    switch (reason) {
        case MIF_CB_DESTROY: {
            int i;
...
void
cm_table2D(Mif_Private_t *mif_private)   /* structure holding parms, inputs, outputs, etc. */
{
    int size, xind, yind;
    ...
    if (mif_private->circuit.init == 1) {
        ...
        *(mif_private->callback) = cm_table2D_callback;
        ...
        if (!loc->state->fp) {
            ...
            if (lbuffer && *lbuffer) {
                p = (char*) malloc(strlen(lbuffer) + strlen(DIR_PATHSEP) + strlen(mif_private->param[4]->element[0].svalue) + 1);
                sprintf(p, "%s%s%s", lbuffer, DIR_PATHSEP, mif_private->param[4]->element[0].svalue);
    ...
    if (mif_private->circuit.anal_type != MIF_AC) {
        ...
        mif_private->conn[2]->port[0]->output.rvalue = outv;
        xderiv = mif_private->param[3]->element[0].rvalue * derivval[0] / xdiff;
         mif_private->conn[2]->port[0]->partial[0].port[0] = xderiv;
        yderiv = mif_private->param[3]->element[0].rvalue * derivval[1] / ydiff;
         mif_private->conn[2]->port[0]->partial[1].port[0] = yderiv;
