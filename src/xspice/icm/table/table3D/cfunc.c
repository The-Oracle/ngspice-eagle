#line 1 "./table/table3D/cfunc.mod"
#include "ngspice/cm.h"
extern void cm_table3D(Mif_Private_t *);
#line 1 "./table/table3D/cfunc.mod"
/*.......1.........2.........3.........4.........5.........6.........7.........8
================================================================================
...
static void
cm_table3D_callback(Mif_Private_t *mif_private, Mif_Callback_Reason_t reason)
{
    switch (reason) {
        case MIF_CB_DESTROY: {
            int i, j;
...
void
cm_table3D(Mif_Private_t *mif_private)   /* structure holding parms, inputs, outputs, etc. */
{
    ...
    if (mif_private->circuit.init == 1) {
        ...
        *(mif_private->callback) = cm_table3D_callback;
        ...
        /* open file */
        loc->state->fp = fopen_with_path(mif_private->param[4]->element[0].svalue, "r");
        loc->state->pos = 0;
        loc->state->atend = 0;
        if (!loc->state->fp) {
            char *lbuffer, *pp;
            ...
            if (lbuffer && *lbuffer) {
                pp = (char*) malloc(strlen(lbuffer) + strlen(DIR_PATHSEP) + strlen(mif_private->param[4]->element[0].svalue) + 1);
                sprintf(pp, "%s%s%s", lbuffer, DIR_PATHSEP, mif_private->param[4]->element[0].svalue);
        ...
        if (!loc->state->fp || fstat(fileno(loc->state->fp), &st)) {
            cm_message_printf("cannot open file %s", mif_private->param[4]->element[0].svalue);
            loc->state->atend = 1;
            loc->init_err = 1;
            return;
        }
        ...
        if (cFile == NULL || cThisLine == NULL) {
            cm_message_printf("Insufficient memory to read file %s", mif_private->param[4]->element[0].svalue);
            loc->state->atend = 1;
            loc->init_err = 1;
            if(cFile) free(cFile);
        ...
        /* generate table core */
        interporder = mif_private->param[0]->element[0].ivalue;
            ...
            while (lLineCount < iy) {
                ...
                if (cThisLinePtr[0] == '*' || cThisLinePtr[0] == '\0') {
                    if (lTotalChars >= lFileLen) {
                        cm_message_printf("Not enough data in file %s", mif_private->param[4]->element[0].svalue);
                        loc->init_err = 1;
                        return;
                    }
                    continue;
    ...
    xval = mif_private->conn[0]->port[0]->input.rvalue;
    yval = mif_private->conn[1]->port[0]->input.rvalue;
    zval = mif_private->conn[2]->port[0]->input.rvalue;

    /* check table ranges */
    if (xval < loc->xcol[0] || xval > loc->xcol[loc->ix - 1]) {
        if (mif_private->param[1]->element[0].ivalue > 0)
...
/* xind yind zind may become too large */
    ...
    if (mif_private->circuit.anal_type != MIF_AC) {
        ...
        outv = mif_private->param[2]->element[0].rvalue + mif_private->param[3]->element[0].rvalue * outval;
        mif_private->conn[3]->port[0]->output.rvalue = outv;
        xderiv = mif_private->param[3]->element[0].rvalue * derivval[0] / xdiff;
         mif_private->conn[3]->port[0]->partial[0].port[0] = xderiv;
        yderiv = mif_private->param[3]->element[0].rvalue * derivval[1] / ydiff;
         mif_private->conn[3]->port[0]->partial[1].port[0] = yderiv;
        zderiv = mif_private->param[3]->element[0].rvalue * derivval[2] / zdiff;
         mif_private->conn[3]->port[0]->partial[2].port[0] = zderiv;

        if (mif_private->param[1]->element[0].ivalue > 1)
            cm_message_printf("\nI: %g, xval: %g, yval: %g, zval: %g, xderiv: %g, yderiv: %g, zderiv: %g", outv, xval, yval, zval, xderiv, yderiv, zderiv);
    } else {
        ac_gain.real = mif_private->param[3]->element[0].rvalue * derivval[0] / xdiff;
        ac_gain.imag= 0.0;
         mif_private->conn[3]->port[0]->ac_gain[0].port[0] = ac_gain;
        ac_gain.real = mif_private->param[3]->element[0].rvalue * derivval[1] / ydiff;
        ac_gain.imag= 0.0;
