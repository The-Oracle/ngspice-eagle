#line 1 "./xtradev/sidiode/cfunc.mod"
                                          ...
                                          inputs, outputs, etc.     */
{
    ...
    if (mif_private->circuit.init==1) {  /* First pass...allocate memory and calculate some constants... */
        ...
        *(mif_private->callback) = cm_sidiode_callback;
        ...
        hRevepsilon = 0.5 * mif_private->param[7]->element[0].rvalue;
        loc->Va = Va = Vrev - hRevepsilon;
        loc->Vb = Vb = Vrev + hRevepsilon;
        if(hRevepsilon > 0.0) {
            loc->a1 = (goff - grev)/mif_private->param[7]->element[0].rvalue;
        ...
        hEpsilon = 0.5 * mif_private->param[6]->element[0].rvalue;
        loc->Vc = Vc = Vfwd - hEpsilon;
        loc->Vd = Vd = Vfwd + hEpsilon;

        if(hEpsilon > 0.0) {
            loc->b1 = (gon - goff)/mif_private->param[6]->element[0].rvalue;
            loc->b2 = gon/Ilimit;
            loc->epsi = MIF_TRUE;
        }
        else
